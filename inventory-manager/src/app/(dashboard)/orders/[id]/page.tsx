  ...
  return (
    <div className="space-y-4">
      ...
      <table className="w-full rounded-lg border bg-white text-sm">
        <thead><tr className="border-b"><th className="p-3 text-left">Product</th><th>Qty</th><th>Unit Price</th></tr></thead>
        <tbody>{order.items.map((i) => <tr className="border-b" key={i.id}><td className="p-3">{i.product.name}</td><td>{i.quantity}</td><td>{formatCurrency(Number(i.unitPrice))}</td></tr>)}</tbody>
      </table>
    </div>
  );
}
